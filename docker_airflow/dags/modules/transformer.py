...
class Transformer():
	...
	def get_data_from_mysql(self, engine_sql, engine_postgres):
	...
	def create_dimension_province(self):
		...
		print("INSERTED dim_province TO POSTGRES SUCCESSFULLY")
	...
	def create_dimension_district(self):
		...
		print("INSERTED dim_district TO POSTGRES SUCCESSFULLY")
	...
	def create_dimension_case(self):
		...
		print("INSERTED dim_case TO POSTGRES SUCCESSFULLY")		

	def create_province_daily(self):
		df = self.get_data_from_mysql(self.engine_sql,self.engine_postgres)
		column_start_case = ['suspect_diisolasi','suspect_discarded', 'suspect_meninggal',
			'closecontact_dikarantina', 'closecontact_discarded', 'closecontact_meninggal',
			'probable_diisolasi', 'probable_discarded', 'probable_meninggal',
			'confirmation_meninggal','confirmation_sembuh']
		column_end_case = ['id', 'status_name', 'status_detail', 'status']
		df_case_dim = df[column_start_case]
		df_case_dim = df_case_dim[:1]
		df_case_dim = df_case_dim.melt(var_name = "status", value_name = "total")
		df_case_dim = df_case_dim.drop_duplicates("status").sort_values("status")
		df_case_dim['id'] = np.arange(1, df_case_dim.shape[0]+1)
		df_case_dim[['status_name','status_detail']] = df_case_dim["status"].str.split('_', n = 1, expand = True)
		df_case_dim = df_case_dim[column_end_case]

		column_start_daily = ['tanggal', 'kode_kab', 'suspect_diisolasi','suspect_discarded', 'suspect_meninggal',
			'closecontact_dikarantina', 'closecontact_discarded', 'closecontact_meninggal',
			'probable_diisolasi', 'probable_discarded', 'probable_meninggal',
			'confirmation_meninggal','confirmation_sembuh']
		df_district_daily = df[column_start_daily]
		...
		print("INSERTED district_daily TO POSTGRES SUCCESSFULLY")
	...
	def create_district_daily(self):
		...
		column_start_daily= ['tanggal', 'kode_prov', 'suspect_diisolasi','suspect_discarded', 'suspect_meninggal',
			'closecontact_dikarantina', 'closecontact_discarded', 'closecontact_meninggal',
			'probable_diisolasi', 'probable_discarded', 'probable_meninggal',
			'confirmation_meninggal','confirmation_sembuh']
		df_province_daily = df[column_start_daily]
		...
		print("INSERTED province_daily TO POSTGRES SUCCESSFULLY")